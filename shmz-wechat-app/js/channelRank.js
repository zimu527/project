...
function getUserInfo(){
    ...
    BWFRI.OlympicGetMyInfoService({'openid':opId} , resultProcessor);
}
...
function getFireList(){
    var onResult = getFireListOnResult;
    var resultProcessor = {
        'onResult':onResult
    };
    BWFRI.OlympicGetRankListService({} , resultProcessor);
}
function getFireListOnResult(result){
    if(result.code != 0){
        alert('不给力，刷新重试~');
    }else {
        // 新建整个表
